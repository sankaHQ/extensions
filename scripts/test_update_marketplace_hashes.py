...
def test_hash_updater_records_each_manifest_dependency_closure(tmp_path: Path) -> None:
    ...
    manifests = update_manifests(tmp_path, release_tag="extensions-v0.1.0a27")
    ...
    for package, payload in manifests.items():
        ...
        assert all(
            wheel["url"].startswith(
                "https://github.com/sankaHQ/extensions/releases/download/extensions-v0.1.0a27/"
            )
...
def test_hash_updater_rejects_an_incomplete_or_wrongly_tagged_wheel_set(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="complete marketplace wheel set"):
        update_manifests(tmp_path, release_tag="extensions-v0.1.0a27")
    with pytest.raises(RuntimeError, match=r"extensions-v0\.1\.0a27"):
...
@pytest.mark.parametrize(
    ...
    [
        ("sanka-extension-drf-to-fastapi", "0.1.0a14"),
        ("sanka-extension-drf-to-flask", "0.1.0a8"),
    ],
)
def test_converter_dependency_closures_resolve_without_an_index(
    ...
    assert "sanka-code-migration==0.1.0a1" in result.stderr
...
def test_release_validator_rejects_invalid_release_boundaries(
    ...
    if case == "connector_entry_point":
        ...
        fastapi = release / "sanka_extension_drf_to_fastapi-0.1.0a14-py3-none-any.whl"
        _metadata_wheel(
            release,
