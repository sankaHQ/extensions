...
Keep `SANKA_CONNECTORS_PUBLISH_ENABLED` and
`SANKA_CONNECTORS_BOOTSTRAP_ENABLED` absent or `false` outside an explicitly
...
3. Dispatch **Publish connector packages** against that exact tag, using
   TestPyPI first.
