...
def flow_request(request):
    payload = json.loads((Path(__file__).parent / "fixtures" / request.param).read_text())
    ...
    return flow.BlueprintRequest(
        "generate-1",
        blueprint.extension,
        flow.create(type=blueprint.origin.identity.id, parameters={"language": "ja"}),
        flow.TargetIdentity("synthetic-workspace", "revision-1", blueprint.required_capabilities),
        blueprint.references,
        values,
        template=blueprint.origin.identity,
        output_schema=blueprint.schema_version,
    )
...
def blueprint(flow_request):
    fixture = (
        "synthetic_sales_quote_blueprint.json"
        if flow_request.output_schema == flow.BLUEPRINT_SCHEMA_VERSION
...
def capability(flow_request):
    return flow.FlowCapability(
        ...
        flow_request.template,
        flow_request.output_schema,
    )
...
@pytest.mark.parametrize("field", ["id", "revision", "capabilities"])
def test_target_snapshot_is_required_and_bound_to_response(flow_request, blueprint, field):
    response = flow.BlueprintResponse.success(flow_request, blueprint)
    payload = flow_request.to_dict()
    del payload["target"][field]
    with pytest.raises(ValueError, match="unexpected or missing"):
        flow.BlueprintRequest.from_dict(payload)
    payload = flow_request.to_dict()
    payload["target"][field] = [] if field == "capabilities" else "changed"
    altered = flow.BlueprintRequest.from_dict(payload)
    assert altered.digest != flow_request.digest
    with pytest.raises(ValueError, match="different requested input"):
        response.validate_for(altered)


@pytest.mark.parametrize("field", ["revision", "digest"])
def test_template_revision_and_digest_are_exact(flow_request, blueprint, capability, field):
    payload = flow_request.to_dict()
    payload["template"][field] = "sha256:" + "f" * 64 if field == "digest" else "other-revision"
    altered = flow.BlueprintRequest.from_dict(payload)
    assert altered.digest != flow_request.digest
    with pytest.raises(ValueError, match="declared template"):
        capability.validate_request(altered)
    with pytest.raises(ValueError, match="requested template"):
        flow.BlueprintResponse.success(altered, blueprint)


def test_missing_native_identity_capability_rejects_both_graph_versions(flow_request, blueprint):
    payload = flow_request.to_dict()
    payload["target"]["capabilities"].remove("flow.record-identity/v1")
    altered = flow.BlueprintRequest.from_dict(payload)
    output = blueprint.to_dict()
    output["parameters"] = altered.blueprint_parameters
    with pytest.raises(
        ValueError, match=r"Target lacks required Flow capabilities: flow\.record-identity/v1"
    ):
        flow.BlueprintResponse.success(altered, flow.Blueprint.from_dict(output))


def test_output_version_cannot_be_changed_by_either_party(flow_request, blueprint, capability):
    payload = flow_request.to_dict()
    payload["output_schema"] = (
        flow.BLUEPRINT_V2_SCHEMA_VERSION
        if flow_request.output_schema == flow.BLUEPRINT_SCHEMA_VERSION
        else flow.BLUEPRINT_SCHEMA_VERSION
    )
    altered = flow.BlueprintRequest.from_dict(payload)
    with pytest.raises(ValueError, match="declared template or output schema"):
        capability.validate_request(altered)
    with pytest.raises(ValueError, match="requested output schema"):
        flow.BlueprintResponse.success(altered, blueprint)


def test_capabilities_are_canonical_but_duplicates_fail(flow_request):
    payload = flow_request.to_dict()
    payload["target"]["capabilities"].reverse()
    assert flow.BlueprintRequest.from_dict(payload).digest == flow_request.digest
    payload["target"]["capabilities"].append(payload["target"]["capabilities"][0])
