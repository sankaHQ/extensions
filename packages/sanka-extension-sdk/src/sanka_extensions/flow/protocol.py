...
from sanka_extensions.flow.blueprint import BLUEPRINT_SCHEMA_VERSION, Blueprint
...
class FlowCapability(WireRecord):
    ...
    output_schema: Literal["sanka-flow-blueprint/v1"] = "sanka-flow-blueprint/v1"

    def __post_init__(self) -> None:
        FlowDefinition(type=self.type)
        choice(self.output_schema, (BLUEPRINT_SCHEMA_VERSION,), "Flow output schema")
        references = unique(
    ...
    def from_dict(cls, value: object) -> Self:
        payload = object_fields(
            value, {"type", "references", "values", "output_schema"}, "Flow capability"
        )
        return cls(
            payload["type"],
...
class TargetIdentity(WireRecord):
    ...
    @classmethod
    def from_dict(cls, value: object) -> Self:
        return cls(**object_fields(value, {"id", "revision"}, "target identity"))
...
class BlueprintResponse(WireRecord):
    ...
    def validate_for(self, request: BlueprintRequest) -> None:
        ...
        if (
            ...
            or self.blueprint.origin.identity.id != request.definition.type
        ):
