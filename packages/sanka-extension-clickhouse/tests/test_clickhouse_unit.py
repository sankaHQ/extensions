...
from sanka_extension_clickhouse import (
    EXTENSION,
...
def test_registration_shape() -> None:
    assert EXTENSION.name == "clickhouse"
    assert EXTENSION.source is None
    assert EXTENSION.destination is not None
    assert isinstance(EXTENSION.destination, DestinationConnector)
    assert isinstance(EXTENSION.destination, SupportsBatchWrites)
