...
from sanka_extension_markdown import EXTENSION, MarkdownSource
...
async def test_count_capability_and_registration(tmp_path: Path) -> None:
    ...
    assert count == 3
    assert EXTENSION.name == "markdown"
    assert EXTENSION.source is not None and EXTENSION.destination is None
