...
- The SPI bundles ``conflict_policy`` / ``identity_fields`` /
  ...
  :class:`sanka_connector.WriteOptions` instead of separate keyword
  arguments.

Optional provisioning fields the SDK leaves to the connector when omitted:

- :class:`sanka_connector.PipelineStage` ``probability`` is used verbatim on
  ...
  do not.
- :class:`sanka_connector.CustomObjectDefinition` ``properties`` are created
