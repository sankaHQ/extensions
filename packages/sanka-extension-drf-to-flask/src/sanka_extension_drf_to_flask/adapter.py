...
def _scan(root: Path, config: dict[str, JsonValue]) -> dict[str, Any]:
    ...
    def visit(patterns: Any, prefix: str = "", parent_supported: bool = True) -> None:
        for pattern in patterns:
            ...
            stock_root = view is APIRootView and parent_supported
            stock_router = bool(actions) and parent_supported
            if stock_router:
                from .viewsets import handler as viewset_handler

                # Only the two stock router patterns, never arbitrary source regexes.
                tail = str(pattern.pattern)
                if tail.endswith("/$") and tail.startswith("^"):
                    converted = tail[1:-1].replace("(?P<pk>[^/.]+)", "<string:pk>")
                    path = _flask_path(prefix + converted)
                    if path:
                        path = path.replace("<string:pk>", "<drf_pk:pk>")
                suffix = r"\.(?P<format>[a-z0-9]+)/?$"
                if tail.startswith("^") and tail.endswith(suffix):
                    converted = tail[1 : -len(suffix)].replace("(?P<pk>[^/.]+)", "<string:pk>")
                    base = _flask_path(prefix + converted)
                    if base:
                        path = base.replace("<string:pk>", "<drf_pk:pk>") + "<drf_suffix:format>"
                if "get" in methods and "head" not in methods:
                    methods.append("head")
                if "options" not in methods:
                    methods.append("options")
            if stock_root:
                from .viewsets import root_handler

                path = "/" + raw.replace(
                    "<drf_format_suffix:format>", "<drf_suffix:format>"
                ).lstrip("/")
            for method in methods or ["get"]:
                ...
                if stock_router:
                    ...
                    source, reason = viewset_handler(
                        view,
                        actions.get(method, actions.get("get", "")),
                        method.upper(),
                        callback.initkwargs,
                    )
                if stock_root:
                    source, reason = root_handler(callback, method.upper())
                initialization = getattr(callback, "initkwargs", {})
                if (
                    initialization
                    and not (
                        (stock_router or stock_root)
                        and set(initialization)
                        <= {"suffix", "detail", "basename", "name", "description", "api_root_dict"}
                    )
                ) or pattern.default_args:
                    source, reason = (
                        None,
    ...
    return {
        "contract_changes": [
            (
                "The generated ModelViewSet API serves JSON. DRF's browsable HTML "
                "API and form uploads are not generated."
            )
        ]
        if any("ModelInput" in (r["source"] or "") for r in routes)
        else [],
...
def _render(scan: dict[str, Any]) -> dict[str, str]:
    pieces = [
        r"""# Generated by Sanka DRF-to-Flask. Repair the gaps listed in migration-gaps.json.
...
class _DRFPk(BaseConverter):
    regex = r"[^/.]+"
class _DRFSuffix(BaseConverter):
    regex = r"\.[a-z0-9]+/?"
    def to_python(self, value):
        return value.lstrip('.').rstrip('/')
app.url_map.converters['drf_pk'] = _DRFPk
app.url_map.converters['drf_suffix'] = _DRFSuffix
...
"""
    ...
    return {
        ...
        "sanka_model.py": Path(__file__)
        .with_name("model_runtime.py")
        .read_text()
        .replace("from .native_runtime import", "from sanka_native import"),
