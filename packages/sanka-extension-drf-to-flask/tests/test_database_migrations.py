...
def test_postgresql_identity_and_pattern_index_contract_is_exact():
    import types
    ...
    payload = schema()
    payload["dialect"] = "postgresql"
    payload["tables"][0]["columns"][0]["identity"] = {"always": False}
    payload["tables"][0]["indexes"].append(
        {
            "name": "item_name_like",
            "columns": ["name"],
            "descending": [False],
    ...
    files = render_database(payload)
    migration_name = next(name for name in files if name.startswith("migrations/versions/"))
    assert "sa.Identity(always=False)" in files["models.py"]
    assert "postgresql_ops={'name': 'varchar_pattern_ops'}" in files["models.py"]
    assert "postgresql_ops={'name': 'varchar_pattern_ops'}" in files[migration_name]

    models = types.ModuleType("models")
    exec(files["models.py"], models.__dict__)
    assert "GENERATED BY DEFAULT AS IDENTITY" in str(
        CreateTable(models.TABLES["item"]).compile(dialect=postgresql.dialect())
    )
    sys.modules["models"] = models
    database: dict[str, object] = {}
    exec(files["database.py"], database)
    bind = types.SimpleNamespace(dialect=postgresql.dialect())
    identifier = models.TABLES["item"].c.id
    reflected_identity = {
        "always": False,
        "start": 1,
        "increment": 1,
    ...
    assert database["_matches_identity"](
        bind, identifier, {"default": None, "identity": reflected_identity}
    )
    assert not database["_matches_identity"](
        bind,
        identifier,
        {"default": None, "identity": {**reflected_identity, "always": True}},
    )
    expected_index = next(
        index for index in models.TABLES["item"].indexes if index.name == "item_name_like"
    )
    actual_index = {
        "name": "item_name_like",
        "column_names": ["name"],
        "column_sorting": {},
        "unique": False,
        "dialect_options": {"postgresql_ops": {"name": "varchar_pattern_ops"}},
    }
    expected_signature = database["_expected_index_signature"](expected_index)
    assert database["_actual_index_signature"](bind, actual_index) == expected_signature
    actual_index["dialect_options"] = {}
    assert database["_actual_index_signature"](bind, actual_index) != expected_signature


def test_postgresql_identity_and_pattern_index_reject_unowned_variants():
    import pytest

    identity = schema()
    identity["dialect"] = "postgresql"
    identity["tables"][0]["columns"][0]["identity"] = {"always": True}
    identity.pop("schema_hash")
    encoded = json.dumps(identity, sort_keys=True, separators=(",", ":"), allow_nan=False)
    identity["schema_hash"] = "sha256:" + hashlib.sha256(encoded.encode()).hexdigest()
    with pytest.raises(ValueError, match="BY DEFAULT AutoField identity"):
        render_database(identity)

    opclass = schema()
    opclass["tables"][0]["indexes"][0]["opclasses"] = ["varchar_pattern_ops"]
    opclass.pop("schema_hash")
    encoded = json.dumps(opclass, sort_keys=True, separators=(",", ":"), allow_nan=False)
    opclass["schema_hash"] = "sha256:" + hashlib.sha256(encoded.encode()).hexdigest()
    with pytest.raises(ValueError, match="operator classes require PostgreSQL"):
        render_database(opclass)
