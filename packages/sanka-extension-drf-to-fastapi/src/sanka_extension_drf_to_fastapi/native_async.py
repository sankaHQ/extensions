        ...
        for field in resource.get("fields", []):
            ...
            if fk and table in specs:
                specs[table]["columns"].setdefault(
                    ...
                    {
                        "name": fk,
                        "kind": specs[str(resource["db_table"])]["columns"][
                            str(resource.get("pk_attname") or "id")
                        ]["kind"],
                        "pk": False,
                        "max_length": None,
                        "allow_null": False,
...
def _add_model_spec(specs: dict[str, dict[str, Any]], item: dict[str, Any]) -> None:
    ...
    for field in [*item.get("fields", []), *item.get("storage", [])]:
        ...
        columns[name] = {
            ...
            "uuid_default": bool(field.get("uuid_default")),
        }
    specs[table] = {
        "table": table,
...
def _render_models(sql_engine: str, manifest: dict[str, Any]) -> str:
    ...
    if sql_engine == "tortoise":
        lines = [
            ...
            "",
            "class DjangoUUIDField(fields.UUIDField):",
            "    def to_db_value(self, value, instance):",
            "        if value is None:",
            "            return None",
            "        value = value if isinstance(value, UUID) else UUID(str(value))",
            "        postgres = instance._meta.db.capabilities.dialect == 'postgres'",
            "        return str(value) if postgres else value.hex",
            "",
        ]
        for spec in specs:
...
def _tortoise_field(column: dict[str, Any]) -> str:
    kind = column["kind"]
    integer_type = "BigIntField" if kind == "big_integer" else "IntField"
    if kind in {"uuid", "related_uuid"}:
        uuid_args = ["primary_key=True"] if column.get("pk") else []
        uuid_args.append("default=None")
        if column.get("allow_null"):
            uuid_args.append("null=True")
        return "DjangoUUIDField(" + ", ".join(uuid_args) + ")"
    if column.get("pk"):
        return f"fields.{integer_type}(primary_key=True)"
    args: list[str] = []
    if column.get("allow_null"):
...
def _sqlalchemy_column(column: dict[str, Any]) -> str:
    kind = column["kind"]
    integer_type = (
        'BigInteger().with_variant(Integer, "sqlite")' if kind == "big_integer" else "Integer"
    )
    if kind in {"uuid", "related_uuid"}:
        options = (
            "primary_key=True"
            if column.get("pk")
            else ("nullable=True" if column.get("allow_null") else "nullable=False")
        )
        return f"mapped_column(Uuid, {options})"
    if column.get("pk"):
        return f"mapped_column({integer_type}, primary_key=True)"
    nullable = "nullable=True" if column.get("allow_null") else "nullable=False"
    if kind == "boolean":
        return f"mapped_column(Boolean, {nullable})"
    if kind in {"integer", "big_integer", "related_pk"}:
        return f"mapped_column({integer_type}, {nullable})"
    if kind == "decimal":
        digits = int(column.get("max_digits") or 12)
        places = int(column.get("decimal_places") or 2)
...
def _render_store(sql_engine: str) -> str:
    ...
    store = {
        ...
        "sqlalchemy": _SQLALCHEMY_STORE,
        "psycopg": _PSYCOPG_STORE,
    }[sql_engine]
    declaration = "async def create_row(resource: dict[str, Any], data: dict[str, Any]) -> Any:\n"
    return (
...
def _creation_values(resource: dict[str, Any], data: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in resource.get("fields", ()):
        if field["kind"] not in {"uuid", "related_uuid", "boolean"}:
            continue
        column = field.get("attname") or field["name"]
        if column not in values:
            if field.get("uuid_default"):
                values[column] = uuid4()
            elif field.get("has_default"):
                values[column] = field["default"]
        value = values.get(column)
        if field["kind"] != "boolean" and value is not None and not isinstance(value, UUID):
            values[column] = UUID(str(value))
    return values
"""
...
def _lookup(resource: dict[str, Any]) -> tuple[str, str]:
    ...
    if lookup == "pk":
        pk = _pk(resource)
        kind = next((f["kind"] for f in resource.get("fields", ())
                     if (f.get("attname") or f["name"]) == pk), "integer")
        return pk, kind
    for field in resource.get("fields") or ():
        if field.get("name") == lookup:
...
def _lookup_value(resource: dict[str, Any], request: Request) -> Any:
    return request.path_params.get(resource.get("lookup_url_kwarg") or resource["lookup"])
...
def _represent(spec: dict[str, Any], value: Any) -> Any:
    if spec["kind"] in {"uuid", "related_uuid"} and value is not None:
        from uuid import UUID
        return str(value if isinstance(value, UUID) else UUID(str(value)))
    if spec["kind"] == "boolean" and value is not None:
        return bool(value)
    if spec["kind"] == "big_integer" and value is not None:
        return str(value) if spec.get("coerce_to_string") else int(value)
...
def _clean_boolean(spec: dict[str, Any], value: Any) -> tuple[Any, list[str]]:
    normalized = value.lower() if isinstance(value, str) else value
    try:
        if normalized in {"t", "y", "yes", "true", "on", "1", 1, True}:
            return True, []
        if normalized in {"f", "n", "no", "false", "off", "0", 0, False}:
            return False, []
        if spec["allow_null"] and normalized in {"null", "", None}:
            return None, []
    except TypeError:
        pass
    return None, [spec["messages"]["invalid"]]


def _clean_uuid(spec: dict[str, Any], value: Any) -> tuple[Any, list[str]]:
    from uuid import UUID
    try:
...
def _field_kind(resource: dict[str, Any], name: str) -> str:
    if name == "pk":
        name = str(resource.get("pk_attname") or "id")
    spec = next((field for field in resource["fields"]
                 if (field.get("attname") or field["name"]) == name), None)
    if spec is None and name == (resource.get("pk_attname") or "id"):
        return "integer"
...
def _comparable(kind: str, value: Any) -> Any:
    ...
    if kind in {"integer", "big_integer", "related_pk", "boolean"}:
        return int(value)
    return str(value)
...
def _validate_scalar_fields(
    ...
    for spec in fields:
        ...
        if name not in payload:
            ...
            elif spec.get("has_default") and not spec.get("default_on_create_only") and not partial:
                value = spec["default"]
                if spec["kind"] == "uuid" and value is not None:
                    from uuid import UUID
                    value = UUID(str(value))
                validated[spec.get("attname") or name] = value
            continue
        raw = payload[name]
        if raw is None:
            if spec["allow_null"]:
