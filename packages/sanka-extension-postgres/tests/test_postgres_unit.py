...
from sanka_extension_postgres import EXTENSION, PostgresDestination, PostgresSource
from sanka_extension_postgres._base import (
    SKIPPED_BINARY,
    cursor_param,
    cursor_text,
    field_family,
...
from sanka_extension_postgres._destination import (
    _column_type,
    _index_name,
    _required_type,
    _write_param,
...
def test_registration_shape_and_capabilities() -> None:
    assert EXTENSION.name == "postgres"
    assert EXTENSION.source is not None and EXTENSION.destination is not None
